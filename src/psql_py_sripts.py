...
def search_by_title(template,dbname,user,passw,server = 'localhost'):
    q = "SELECT * FROM paper WHERE title LIKE '%"+ template +"%'"
    return do_query(q,dbname,user,passw)

def search_by_author(template,dbname,user,passw,server = 'localhost'):
    q = "SELECT * FROM paper AS p, author AS a WHERE p.pid = a.aid AND a.name = '" + template + "'"
    return do_query(q,dbname,user,passw)
    
def search_by_venue(template,dbname,user,passw,server = 'localhost'):
    q = "SELECT * FROM paper AS p, venue as v WHERE p.venue_id = v.vid AND v.name LIKE '%"+ template +"%'"
    return do_query(q,dbname,user,passw)
    
def search_by_keyword(template,dbname,user,passw,server = 'localhost'):
    q = "SELECT p.* FROM paper AS p, keyword AS k, contains AS c WHERE p.pid = c.paper_id AND k.kid = c.keyword_id AND k.value = LIKE '%"+ template +"%'"
    return do_query(q,dbname,user,passw)
    
def search_related_article(template,dbname,user,passw,server = 'localhost'):
    q = "SELECT * FROM paper AS p WHERE pid IN ( SELECT to_id FROM refs WHERE from_id = " + template + ")"
    return do_query(q,dbname,user,passw)
